...
double * bcg_prec(double** A, double* b, double* x0, int n, int & cnt, double** M) {
  double r1[n];
  double r2[n];
  ...
  for (int j = 0; j < 300; j++) {
    cnt++;
    double* z1 = LUSolve(M , r1, n);
    double* z2 = LUSolve(MT, r2, n, true);
    double q1[n];
    double q2[n];
    double betha;
    double alpha;
    rho[j%2] = mult_vector_to_vector(r2, z1, n);
    ...
    for (int i = 0; i < n; i++) {
      p1[i] = z1[i] + betha*p1[i];
      p2[i] = z2[i] + betha*p2[i];
    }
    delete[] z1;
    delete[] z2;
