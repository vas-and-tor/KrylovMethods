...
double * fom_prec(double** A, double* b, double* x0, int n, int m, double** M) {
  double* r0;
  double betha;
...
int main(void) {
  ...
  //for (int i = 0; i < n; i++) {
    //cin >> b[i];
  //}
  A = read_MatrixMarket("tests/cage10.mtx", n);
  b = new double[n];
  for (int i = 0; i < n; i++) {
  ...
  while (diff(A, b, xm, n) > 1e-6) {
    if (cnt >= 300) break;
    cnt++;
    double* x = fom_prec(A, b, xm, n, 10, M);
    for (int i = 0; i < n; i++) {
      xm[i] = x[i];
    }
    cerr << "Answer: ";
